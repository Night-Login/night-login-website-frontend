...
import type { OnboardingStatusResponse } from "@/utils/types/api";
...
export const authOptions: NextAuthOptions = {
      ...
      async authorize(credentials) {
        try {
          ...
          console.error(
            ...
            axios.isAxiosError(error) ? error.code : "unknown",
          );
          return null;
        }
      },
  ...
  callbacks: {
    async signIn({ user, account }) {
      ...
      if (account?.provider === "google" || account?.provider === "github") {
        try {
          ...
          const response = await axios.post(
            `${backendUrl()}/api/v1/user/oauth-login`,
            {
              email: user.email,
              name: user.name,
          ...
          if (
            ...
            user.onboardingCompleted =
              response.data.data.user?.onboardingCompleted || false;
            return true;
          }
          ...
          console.warn(
            "Backend OAuth endpoint not available, using NextAuth session only",
          );
          return true;
        } catch (error) {
          console.error(
    ...
    async jwt({ token, user, account, trigger }) {
      // Initial sign in
      if (user) {
        // Google/GitHub access tokens must not be sent to our backend as API tokens.
      ...
      if (trigger === "update" && token.accessToken && backendUrl()) {
        try {
          const response = await axios.get<OnboardingStatusResponse>(
            `${backendUrl()}/api/v1/onboarding/status`,
            {
          ...
          if (typeof status?.onboardingCompleted === "boolean") {
            token.onboardingCompleted = status.onboardingCompleted;
          }
          if (typeof status?.role === "string") token.role = status.role;
        } catch {
          // Keep the last verified claims if the backend cannot confirm an update.
          console.error("Unable to refresh onboarding session from backend");
        }
      }
      return token;
    },
  ...
  // Avoid logging session tokens or credential payloads.
  debug: false,
};
