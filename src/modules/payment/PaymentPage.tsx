...
import type { CreatePaymentResponse } from "@/utils/types/api";
  ...
  const handleStatusCheck = async () => {
    try {
      ...
      const response = await api.get(
        `/api/v1/transaction/check-payment?orderId=${encodeURIComponent(orderId)}`,
      );
      ...
      if (
        ...
        toast.warning(
          "Payment not yet completed. Please complete the payment.",
        );
      } else {
