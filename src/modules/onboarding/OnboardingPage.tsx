...
export default function OnboardingPage() {
      ...
      if (status === "authenticated" && session?.user?.id) {
        try {
          ...
          if (response.data.onboardingCompleted) {
            ...
            if (!refreshed?.user.onboardingCompleted) {
              throw new Error(
                "Unable to refresh onboarding status. Please sign in again.",
              );
            }
  ...
  }, [status, session?.user?.id, router, update]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      (selectedRole === "admin" || selectedRole === "leader") &&
      !verificationCode
    ) {
    ...
    try {
      ...
      // Refresh from the backend on the server, never trust a client-supplied role.
      const refreshed = await update();
      if (!refreshed?.user.onboardingCompleted) {
        throw new Error(
    ...
    const commonFields = (
      <>
        <div className="flex flex-col gap-2">
          ...
          <input
            className="w-full bg-[#F3F3F3] px-6 py-3 rounded-lg focus:outline-hidden"
            type="tel"
        ...
        <div className="flex flex-col gap-2">
          ...
          <textarea
            className="w-full bg-[#F3F3F3] px-6 py-3 rounded-lg focus:outline-hidden resize-none"
            rows={3}
          ...
          <>
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                onChange={(e) =>
                  handleInputChange("lecturerId", e.target.value)
                }
              />
            </div>
            <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                onChange={(e) =>
                  handleInputChange("officeRoom", e.target.value)
                }
              />
            </div>
          </>
        ...
        return (
          <>
            ...
            <div className="flex flex-col gap-2 bg-red-50 p-4 rounded-lg border border-red-200">
              ...
              <input
                className="w-full bg-white px-6 py-3 rounded-lg focus:outline-hidden border border-red-200 focus:border-red-500"
                type="password"
              ...
              <p className="text-sm text-red-600">
                🔒 Admin role requires a verification code. Contact your
                administrator.
              </p>
            </div>
          </>
        ...
        return (
          <>
            ...
            <div className="flex flex-col gap-2 bg-red-50 p-4 rounded-lg border border-red-200">
              ...
              />
              <p className="text-sm text-red-600">
                🔒 Leader role requires a verification code. Contact your
                administrator.
              </p>
            </div>
          </>
        ...
        <div className="max-w-4xl w-full">
          ...
          <div className="flex justify-center mb-12">
            <div className="flex items-center gap-4">
              <div
                className={`flex items-center justify-center w-10 h-10 rounded-full ${step >= 1 ? "bg-red text-white" : "bg-gray-300"}`}
              >
                1
              </div>
              <div
                className={`w-20 h-1 ${step >= 2 ? "bg-red" : "bg-gray-300"}`}
              ></div>
              <div
                className={`flex items-center justify-center w-10 h-10 rounded-full ${step >= 2 ? "bg-red text-white" : "bg-gray-300"}`}
              >
                2
              </div>
            </div>
          ...
          {step === 2 && selectedRole && (
            <div className="bg-white rounded-xl shadow-lg p-8 max-w-2xl mx-auto">
              ...
              <h2 className="text-2xl font-bold mb-2">
                Complete Your{" "}
                {roles.find((r) => r.value === selectedRole)?.title} Profile
              </h2>
