...
import {
  ...
  dummyOrganizations,
  dummyGrades,
} from "@/modules/talent-pool/data/TalentData";
import Chips, {
  getEducationVariant,
  getEducationLabel,
} from "@/modules/talent-pool/components/Chips";
...
const Modal: React.FC<ModalProps> = ({ member, isOpen, onClose }) => {
  ...
  const skillsByCategory = member.skills.reduce(
    (acc, skill) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm transition-all">
      ...
      >
        ...
        <div className="w-full md:w-2/5 bg-linear-to-br from-red to-dark-2 relative flex flex-col">
          {/* Close button */}
          <button
            onClick={onClose}
